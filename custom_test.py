...
def cropEye(img, eyeInfo, index):
    height = img.shape[0]
    y = height - eyeInfo["y"][index]
    tl_x = int(eyeInfo["x"][index] - eyeInfo/2)
    tl_y = int(y + eyeInfo/2)
    br_x = int(tl_x + eyeInfo)
    br_y = int(tl_y - eyeInfo)
...
def create_dataset(basepath, savefile):
    ...
    for i in filenames:
        ...
        print(eye_size)
        # get left eye
        left_eye = cropEye(img, left_info, index)
        # get right eye
        right_eye = cropEye(img, right_info, index)

        face_detect = detect_faces(haar_face_cascade, img)
        face_grid = np.zeros(shape=(1, 25, 25), dtype=np.float32)
        if face_detect != ():
            tl_x = min(25, max(1, face_detect[0]))
            tl_y = min(25, max(1, face_detect[1]))
            br_y = min(25, max(1, tl_y - face_detect[2])) 
            br_x = min(25, max(1, tl_x + face_detect[3]))
            face_grid[0, tl_y:br_y, tl_x:br_x]
